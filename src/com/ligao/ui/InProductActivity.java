 ...
 * 
 */
public class InProductActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.in_product_list);
	...
	Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (msg.what == 0x02) {// 查询垛数量
				...
				ToastUtil.showToast(InProductActivity.this, "服务器链接错误!");
			}
		}
	...
	private void existsBarCodeExecute(String re) {
		...
		if("0".equals(re)){
			...
			DiaLogUtils.showDialog(InProductActivity.this, re, false);
		}
	}
	/**
	 * 根据箱码获取所在垛的箱码集合
	...
	private void saveOutWareHouseExecute(String re){
		if(re.indexOf("操作成功") != -1){
			   ...
			   SpUtil.putString(getApplicationContext(), Constants.FINISH_OUT_ORDERS, finishOutOrders);
			   	DiaLogUtils.showDialog(InProductActivity.this, "操作成功!", false);
			}else{
				DiaLogUtils.showDialog(InProductActivity.this, re , false);
	...
	private void outWarehouse() {
		if(outOrder.getState()==2 &&"1".equals(outOrder.getHandStatus())){
			...
			DiaLogUtils.showDialog(InProductActivity.this,"没有扫描条码，不可出库!", false);
		}
	}
	...
	private void operation() {
		new AlertDialog.Builder(InProductActivity.this)
				.setTitle("操作提示")
				// 设置对话框标题
				.setMessage("请选择操作类型！")
				// 设置显示的内容
	...
	private boolean VerificationBarCode(String barCode){
           ...
           {
        	   DiaLogUtils.showDialog(InProductActivity.this,"扫瞄条码格式不正确!", false);
        	   panduan = false;
        	   return false;
           }
           ...
           if (!exist)
           {
               DiaLogUtils.showDialog(InProductActivity.this,"扫瞄条码不在出库产品中!", false);
               panduan = false;
               return false;
           }
           return panduan;
